var keyf = "&api_key=7z1kztd7F7pXT3NNFBQWfOzfXEuwUEqy";
...
const handle_data = function(res) {
    ...
    for (let i = 0; i < user_arr.length; i += 1) {
        ...
        new_li_elem.style.width = "20vw";
        new_li_elem.style.height = "20vh"
...
const search_fn = function() {
    ...
    fetch(`https://api.giphy.com/v1/gifs/search?${keyf}&q=${user_id}`).then(convert_to_json).then(handle_data);
}
...
fetch(`https://api.giphy.com/v1/gifs/trending?${keyf}`).then(convert_to_json).then(handle_data)